#!/usr/bin/env python3
"""对单张或多张图片做 YOLOv8 目标检测推理（Ultralytics，与训练 run 一致）。"""
...
DEFAULT_WEIGHTS = REPO_ROOT / "models" / "yolov8_fruit_detect_best.pt"
...
def main() -> int:
    p = argparse.ArgumentParser(description="YOLOv8n 水果检测推理（formal100 run）")
    p.add_argument(
        "--weights",
        type=Path,
